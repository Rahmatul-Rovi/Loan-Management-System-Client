...
const MyLoans = () => {
          ...
          if (app.status === "disbursed" && app.repayStatus !== "paid" && app.deadline) {
            ...
            if (today > deadlineDate) {
              ...
              return {
                ...
                payableWithPenalty: Number(app.repayAmount) + totalPenalty,
              };
            }
          }
          return app;
        });
        setApplications(updatedData.reverse());
      } else {
        setApplications([]);
      }
    } catch (err) {
  ...
  const remainingRepayment = applications
    ...
    .reduce((sum, app) => sum + Number(app.isOverdue ? app.payableWithPenalty : app.repayAmount || 0), 0);
  ...
  return (
    <div className={`min-h-screen p-6 ${theme === "dark" ? "bg-[#0A122A] text-white" : "bg-gray-50 text-gray-800"}`}>
              ...
              {applications.map((app) => (
                <tr key={app._id} className="border-b dark:border-gray-700 hover:bg-indigo-500/5 transition">
                  ...
                  <td className="px-4 py-4">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${app.isOverdue ? "bg-red-600 text-white animate-pulse" : app.status === "disbursed" ? "bg-blue-100 text-blue-700" : app.status === "approved" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                      {app.isOverdue ? "Overdue" : app.status}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    {app.repayStatus === "paid" ? (
                      ...
                      <div className="flex flex-col gap-1">
                        <button onClick={() => handlePayClick(app)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-md text-[10px] font-bold shadow-sm transition transform hover:scale-105 active:scale-95">Pay Amount</button>
                        {app.isOverdue && <span className="text-[10px] text-red-500 font-bold animate-bounce text-center">+${app.penaltyAmount} Fine Included</span>}
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-4 space-x-2">
                    <button
                      onClick={() => { setViewApp(app); setIsViewModalOpen(true); }}
                      className="px-3 py-1 bg-gray-500/20 hover:bg-gray-500 hover:text-white text-xs rounded-lg transition"
                    >View</button>
                    {app.status === "pending" && (
      ...
      {/* --- Loan Details View Modal (Added Correctlly Here) --- */}
      {isViewModalOpen && viewApp && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[10000] p-4 backdrop-blur-md">
          <div className={`w-full max-w-2xl p-8 rounded-3xl shadow-2xl ${theme === "dark" ? "bg-[#111B33] text-white border border-gray-700" : "bg-white text-gray-800"}`}>
            <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
              <h3 className="text-2xl font-black text-indigo-500">Loan Details</h3>
              <button onClick={() => setIsViewModalOpen(false)} className="text-3xl hover:text-red-500">&times;</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <p className="text-sm opacity-60">Loan Title</p>
                <p className="font-bold text-lg">{viewApp.loanTitle}</p>
                <p className="text-sm opacity-60">Applied Amount</p>
                <p className="font-bold text-xl text-green-500">${Number(viewApp.loanAmount).toLocaleString()}</p>
                <p className="text-sm opacity-60">Application Date</p>
                <p className="font-medium">{new Date(viewApp.appliedDate || Date.now()).toLocaleDateString()}</p>
              </div>
              <div className="space-y-3">
                <p className="text-sm opacity-60">Status</p>
                <span className="px-3 py-1 rounded-full text-xs font-bold uppercase bg-indigo-500/20 text-indigo-400">{viewApp.status}</span>
                <p className="text-sm opacity-60">Payable Amount (with interest)</p>
                <p className="font-bold text-lg">${Number(viewApp.repayAmount).toLocaleString()}</p>
                <p className="text-sm opacity-60">Repayment Deadline</p>
                <p className={`font-bold ${viewApp.isOverdue ? 'text-red-500' : ''}`}>{viewApp.deadline ? new Date(viewApp.deadline).toLocaleDateString() : "Not Disbursed Yet"}</p>
              </div>
            </div>
           {/* User Information Section */}
<div className="mt-8 p-4 rounded-2xl bg-black/20 border border-gray-700">
  <h4 className="text-sm font-bold mb-2 uppercase opacity-50">Borrower Info</h4>
  <p className="text-sm">Email: <span className="text-indigo-400">{user?.email || viewApp.userEmail || "N/A"}</span></p>
</div>
            <div className="mt-8 flex justify-end">
              <button onClick={() => setIsViewModalOpen(false)} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition">Close View</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isModalOpen && selectedApp && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[9999] p-4 backdrop-blur-sm">
          <div className={`w-full max-w-md p-8 rounded-3xl shadow-2xl ${theme === "dark" ? "bg-[#111B33] text-white border border-gray-700" : "bg-white text-gray-800"}`}>
            ...
            <div className={`mb-6 p-5 rounded-2xl border ${selectedApp.isOverdue ? "bg-red-500/10 border-red-500/30" : "bg-indigo-500/10 border-indigo-500/20"}`}>
              <p className="text-sm opacity-70">Repaying for: <span className="font-bold">{selectedApp.loanTitle}</span></p>
              <p className="text-2xl font-black mt-2">Total Payable: <span className={selectedApp.isOverdue ? "text-red-500" : "text-green-500"}>${Number(selectedApp.isOverdue ? selectedApp.payableWithPenalty : selectedApp.repayAmount).toLocaleString()}</span></p>
              {selectedApp.isOverdue && <p className="text-xs text-red-400 mt-1 font-bold">* Includes {selectedApp.penaltyAmount} BDT late penalty</p>}
            </div>
...
// Stateless StatCard component (No modal here)
