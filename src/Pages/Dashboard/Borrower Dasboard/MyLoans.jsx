...
const MyLoans = () => {
  ...
  const remainingRepayment = applications
    ...
    .reduce(
      (sum, app) =>
        sum +
        Number(app.isOverdue ? app.payableWithPenalty : app.repayAmount || 0),
      0,
    );
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <StatCard
            ...
            label="Remaining Repay"
            value={`$${remainingRepayment.toLocaleString()}`}
            color="red"
          />
        </div>
          ...
          >
            ...
            >
              <tr>
                ...
                <th className="px-4 py-4 text-left uppercase text-xs">
                  Amount
                </th>
                <th className="px-4 py-4 text-left uppercase text-xs">
                  Status
            ...
            <tbody className={theme === "dark" ? "bg-[#0F172A]" : "bg-white"}>
              {applications.map((app) => (
                <tr
                  key={app._id}
                  className="border-b dark:border-gray-700 hover:bg-indigo-500/5 transition"
                >
                    ...
                    {app.repayStatus === "paid" ? (
                      ...
                      <div className="flex flex-col gap-1">
                        <button
                          ...
                          className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-md text-[10px] font-bold shadow-sm transition transform hover:scale-105 active:scale-95"
                        >
                          Pay Amount
                        </button>
                        {app.isOverdue && (
                          <span className="text-[10px] text-red-500 font-bold animate-bounce text-center">
                            +${app.penaltyAmount} Fine Included
                          </span>
                        )}
                      </div>
                    ) : (
      ...
      {isModalOpen && selectedApp && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[9999] p-4 backdrop-blur-sm">
          <div
            className={`w-full max-w-md p-8 rounded-3xl shadow-2xl ${theme === "dark" ? "bg-[#111B33] text-white border border-gray-700" : "bg-white text-gray-800"}`}
          >
            ...
            <div
              className={`mb-6 p-5 rounded-2xl border ${selectedApp.isOverdue ? "bg-red-500/10 border-red-500/30" : "bg-indigo-500/10 border-indigo-500/20"}`}
            >
              ...
              <p className="text-2xl font-black mt-2">
                ...
                <span
                  className={
                    selectedApp.isOverdue ? "text-red-500" : "text-green-500"
                  }
                >
                  $
                  {Number(
                    selectedApp.isOverdue
                      ? selectedApp.payableWithPenalty
                      : selectedApp.repayAmount,
                  ).toLocaleString()}
                </span>
              </p>
              ...
              {selectedApp.isOverdue && (
                <p className="text-xs text-red-400 mt-1 font-bold">
                  * Includes {selectedApp.penaltyAmount} BDT late penalty
                </p>
              )}
            </div>
...
const StatCard = ({ theme, icon, label, value, color }) => (
  <div
    className={`p-6 rounded-2xl shadow-md flex items-center space-x-4 ${theme === "dark" ? "bg-[#111B33]" : "bg-white border"}`}
  >
    ...
    <div>
      ...
      <h2
        className={`text-2xl font-extrabold ${color === "red" ? "text-red-500" : ""}`}
      >
        {value}
      </h2>
    </div>
