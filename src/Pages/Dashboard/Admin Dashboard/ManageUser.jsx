...
const ManageUser = () => {
  ...
  // Fetch users
  useEffect(() => {
    fetch("https://loan-server-theta.vercel.app/users", {
...
const handleUpdateRole = (userId, newRole) => {
  fetch(`https://loan-server-theta.vercel.app/users/${userId}`, {
    method: "PATCH",
    headers: { 
    ...
    .then((res) => {
      // If response is below 200-299 
      if (!res.ok) throw new Error("Failed to update"); 
      return res.json();
    })
    .then((data) => {
      // Success messsgae if server send data
      Swal.fire({
        icon: "success",
        title: "Role Updated",
